...
| 기능 | 하는 일 | 상태 |
|---|---|---|
| **등록 (REG)** | 제품명 검색 → 네이버 후보 선택 → variant(용량 등) 축 정의 | ✅ M1 진행 |
| **기준가 엔진 (BM)** | 핫딜 3사 수집 → 병합·교차검증 → 정규화 → 이상치 양방향 분리 → median 기준가 + P25 굿딜라인 + 3단 표본 표시 | ✅ M1 진행 |
| **감시·알림 (AL)** | 폴링 → 트리거 판정 → 텔레그램 알림(인라인 버튼 승격) → 승격 큐 | ✅ M1 진행 |
