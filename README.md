...
| **파이프라인 (core)** | `raw_deal_post` → 매칭 → `deal_event` → 가격 재처리 → 종료 판정 (60초 주기) | ✅ `PipelineScheduler`. 매 틱 카운터(OBS-02) + JSON 로그(OBS-01). 스모크가 종단 증명. ⚠️ 2026-07-10까지 **이 트리거가 없어 시스템이 죽어 있었다**([Q-27](docs/91-open-questions.md)) |
...
| 기능 | 하는 일 | 상태 |
|---|---|---|
| **구매 성적표 (PUR)** | 구매 기록 → 관찰 모드(산 뒤에도 알림) → 사후 "호구였나" 성적표 | 🔨 기록·관찰 문맥은 REST + 웹 화면 동작. ⛔ 성적표 발급(REPORT_PENDING→CLOSED) 미구현 |
