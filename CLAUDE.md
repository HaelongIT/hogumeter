...
## 축적된 규칙
- **상한(cap)이 있는 계산은 상한 없는 중간값을 만들지 않는다** — 포화 비교를 곱셈/누적 **전에** 스칼라로 수행. `min(계산값, cap)`은 이미 터진 뒤라 보호막이 아니다. 포화 테스트 입력은 경계 바로 위가 아니라 극단값으로. (2026-07-09, 지수 백오프)
- **외부 시스템의 거절 신호를 재시도 가능한 오류로 분류하지 않는다** — 실패를 단일 개념으로 뭉개지 말고 `OK / 일시장애 / 거절(403·429)`로 나눈다. 일시장애만 백오프하고 거절은 중지+알림(SEC-08). (2026-07-09, 폴링 스케줄러)
