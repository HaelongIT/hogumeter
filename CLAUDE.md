...
- **푸시는 사용자 직접.** 에이전트는 커밋까지만 — `main` 강제 push 금지(`permissions.deny`가 강제).
...
- 정지조건 중 **결정론적인 둘(`git push`·실 네트워크 호출)은 훅으로 강제**된다(`.claude/hooks/guard.sh`, PreToolUse). 나머지("임의"의 판단이 필요한 것)는 이 지침의 몫이다.
