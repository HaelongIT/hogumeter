...
- **[필수]** 오프사이트 **실 버킷·IAM 준비** — `.env`에 `BACKUP_S3_BUCKET`(+ `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_DEFAULT_REGION`). **비워두면 업로드를 건너뛰고 "오프사이트 없음"을 출력한다** — 조용히 성공하지 않는다. IAM은 해당 prefix에 `PutObject`/`GetObject`/`HeadObject`**+`ListBucket`**만(신선도 점검이 `list-objects-v2`를 쓴다). 버킷 버저닝·수명주기(예: 90일) 권장.
  - **올라가고 있는지 확인하는 법**(오프사이트는 **조용히 꺼진다** — `.env`에서 `BACKUP_S3_BUCKET` 한 줄이 사라지면 업로드는 exit 0으로 넘어가고, 로컬 신선도 게이트도 초록이다):
    `bash scripts/check-offsite-freshness.sh` → 최신 원격 객체가 **26시간 이내**인지 본다. **미설정 자체를 실패로 본다** — "설정을 지웠는데 게이트가 초록"이 가장 나쁘다.
  - **[권장]** 로컬 점검과 함께 cron에: `40 4 * * * cd /srv/hogumeter && bash scripts/check-backup-freshness.sh && bash scripts/check-offsite-freshness.sh || <알림>`
  - 이 게이트는 CI `offsite` 잡이 **MinIO에 대고 실제로 돌린다**(통과 1 + 차단 3: 미설정 · 빈 prefix · 나이 계산).
...
- **[필수]** 텔레그램 실 발송 켜기(AL-05, 2026-07-21 어댑터 완성): `.env`에 **`TELEGRAM_ENABLED=true` + `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID`**(알림 나갈 대상). 셋이 다 있어야 실 어댑터가 뜬다 — enabled=true인데 토큰·chat이 비면 **기동이 실패**해 설정 누락을 드러낸다(조용히 스텁으로 안 떨어진다). 확인법: `docker logs`에 `[STUB alert]`가 아니라 실제 전송이 보이는지, 봇 대화창에 알림이 오는지. 관리자 알림용 별도 chat(OBS-03)은 후속(연속 실패 push, Q-56).
- **[권장]** 실 발송 전 수동 스파이크 1회: `HttpTelegramApi`는 fake로만 검증됐다(실 네트워크 테스트 금지). 토큰 발급 후 `getUpdates`로 chat_id 확인 + 테스트 메시지 1건이 실제로 도착하는지 눈으로 본다.
- **[완료]** **SEC-03 인바운드 화이트리스트** — `TELEGRAM_ALLOWED_CHAT_IDS`. **아웃바운드(알림 발송)는 설정된 `TELEGRAM_CHAT_ID` 하나로만** 나가므로 이미 안전하다(2026-07-21). **인바운드**(인라인 버튼 승격/기각/무시 콜백)는 `ReviewCallbackRouter`가 이미 이 목록으로 거른다 — 비면 `TELEGRAM_CHAT_ID`로 폴백하고, 둘 다 없으면 빈 허용집합(아무도 허용 안 함)으로 닫힌다(`docs/91` Q-61 해소, BE-28 코드리뷰 20260806에서 이 문서의 "미소비" 서술이 낡았음을 발견해 정정). 운영 배포 시엔 실제로 허용할 chat_id를 채워 넣을 것.
- **[완료]** ~~봇 토큰을 넣기 전에 `docs/91` Q-27 ③을 고칠 것~~ **해소(2026-07-11)**: 최초 수집 시 이미 품절인 원문의 "지금 사라" 오알림을, `candidateFrom`의 초기 상태를 `post.getStatus()` 기반으로(SOLD_OUT/DELETED→ENDED) + `AlertEvaluator`의 ENDED 억제로 차단했다. 관통 테스트가 send 0회를 단언한다. core 소유권 조율로 수정.
...
### 품절 셀렉터가 죽었는지 확인하는 법 (2026-07-10 추가)

- **`docker logs collector`의 `cycle` 이벤트에서 `sold_out_by_site`를 본다.** golden 기준: 루리웹 3 · 펨코 2 · **뽐뿌 0**.
- **어떤 사이트가 며칠째 0이면 그 사이트의 품절 셀렉터를 의심한다.** 루리웹의 `[종료]`가 정확히 그랬다 — 마커가 제목 앵커 밖에 있어 파서가 볼 수 없었고, **모든 테스트가 GREEN**이었다(golden은 고정이고 사이트는 변한다).
- **뽐뿌의 0은 이미 알려진 미지수다**(Q-19): `.end2`가 골든에 0건이라 SOLD_OUT 경로가 검증된 적 없다. 한 번이라도 관측하면 fixture로 채취한다. 오래 관측해도 안 나오면 셀렉터가 틀린 것이다.
- **알림이 아니라 카운터인 이유**: 뽐뿌가 늘 0이라 알림으로 만들면 매 사이클 오알림한다. 오차단은 사람이 게이트를 꺼 버리게 만든다 — 사실은 세고 결론은 사람이 낸다(절대 원칙 2).
