...
- **[필수 · 선결 · 후속]** **SEC-03 인바운드 화이트리스트** — `TELEGRAM_ALLOWED_CHAT_IDS`. **아웃바운드(알림 발송)는 설정된 `TELEGRAM_CHAT_ID` 하나로만** 나가므로 이미 안전하다(2026-07-21). 남은 건 **인바운드**(인라인 버튼 승격 콜백, Q-15 후속) — 그 핸들러가 생길 때 이 목록 밖 chat_id의 명령을 거부하도록 함께 넣는다(`docs/91` Q-61). 지금은 인바운드 핸들러 자체가 없어 미소비.
