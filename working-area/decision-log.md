...
| 2026-07-04 | `BenchmarkParams`(수치 seam)를 `domain.benchmark` → **`domain` 루트로 이동**. benchmark·deal·matching 공통 참조 공유 커널이라, `benchmark→deal`(BenchmarkCalculator)와 `deal→benchmark`(DealMergePolicy) **패키지 순환**을 끊음 | BM-04 착수 시 순환 발견 | core domain/BenchmarkParams.java, benchmark 임포트 |
