...
## 골자
- ✅ **WatchItem** = `dealEventId` + `anchorPostId`(핀 시점 소스 원문, **재구성 자동 승계 2026-07-30 배선**)
  + 메모. 유일성 = 딜당 활성 핀 1개(부분 유니크 인덱스). 결말 후 재핀 = 같은 dealEventId로 새 행(이력은
...
- **핀 = 매칭 확정 겸함**(매칭 축만) — 미착수(범위 밖, Q-83과 무관). ✅ 핀 이력 딜은 키워드 사후학습
  근거 제외 — 2026-07-30 배선(Q-83 ③).
...
- ✅ **PUR 프리필 해소(2026-07-30)**: [샀어요] → 그 딜의 variant 판단 화면으로 이동 + `PurchasePanel`
  ...
  않는다(판단 화면에서 고르는 기존 흐름 유지). ✅ 부활 미응답 플래그도 2026-07-30 배선(`watch_item.
  revive_unacknowledged` + [확인함]). 자격 상실 확인 알림은 **구조적으로 도달 불가능**(Q-83 ⑤ 참조,
  C-4가 outlierFlag 사후 재평가를 막아 핀 이후 자격 상실이 코드상 일어날 수 없음).
- ✅ **핀 후속(인상 1회) 2026-07-30 배선** — `FollowUpKind.PINNED_PRICE_INCREASED`, 핀 자체가 자격.
  인하·품절·종료·검증까지 핀 기반 무조건으로 여는 건 회귀 위험이 커 채택 안 함(decision-log 참조).
  DealEvent 층의 REOPENED(부활) 후속은 이미 있다(DN-C1, 핀 여부 무관하게 전 딜에 적용).
...
- `docs/18` DIGEST ④(핀 결말 전이 + 부활) — WatchItem은 이제 있으나 DIGEST④ 자체는 여전히 WATCH 표면
  (회고·미확인 섹션) 완성 후로 대기.
- `docs/12` 후속 계열의 핀 딜 인상 특례(1회) — ✅ Q-83 ④ 2026-07-30 배선.
