...
- ✅ **결말 전이**: [샀어요]→BOUGHT / 기각·해제→DROPPED(결과가 같아 상태 하나로 합침, `ResolvePinUseCase`)
  ...
  **PUR 프리필은 모양만 확정됨(2026-07-28, decision-log 참조) — 코드는 아직 없다**: [샀어요] → 그 딜의
  variant 판단 화면으로 이동 + `PurchasePanel` 폼에 딜 가격·오늘·연결 딜 프리필, 사람이 실지불가를
  확인·수정해 [기록]. 수요축 값은 프리필하지 않는다(판단 화면에서 고르는 기존 흐름 유지).
