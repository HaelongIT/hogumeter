...
0-2. **⚠️ 코드 안의 블로커 — `docs/91` Q-46(실측 2026-07-10)**: collector가 뽑는 조건 태그(`카할`·`유료배송(금액미상)`·`조건부무료배송:와우무배`)가 **`raw` jsonb에 갇혀 `deal_event`에 도달하지 않는다.** `deal_event.applied_conditions` 컬럼은 V1에 있지만 `DealEvent` record에 필드가 없고 엔티티도 미매핑이다. 골든 실측 **뽐뿌 9.5% · 펨코 15%** — 폴링을 켜는 순간 표본의 약 1할이 **조건부 가격인데 무조건 가격으로** 기준가에 섞인다. `유료배송(금액미상)`은 배송비 0을 더해 표본이 실제보다 **낮게** 편향된다(진짜 좋은 딜을 놓친다). 화면·알림 어디에도 표시가 없고 로그도 없다. **실 폴링 전 필수.** core 기존 파일이라 조율.
