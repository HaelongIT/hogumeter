...
| AL-03 **후속 알림**(VERIFIED·PRICE_CHANGED·ENDED) | 🔴 **없다.** 위 줄은 "후속 자격 ✅ GREEN"이라 적어 두고 있었으나 **거짓이었다**(2026-07-11 실측): `FollowUpEvaluator`는 존재하고 단위 테스트도 GREEN이지만 **프로덕션 소비처가 0**이고 `FollowUpKind.*`를 만드는 코드도 없다. 딜이 검증되거나 가격이 바뀌거나 종료돼도 사용자는 아무것도 못 듣는다 — 특히 `ENDED`는 "지금 사라"를 받고 달려간 사람에게 **"끝났다"고 말해 줄 유일한 경로**다. 진짜 블로커는 **알림 이력 저장소 부재**(`alreadyAlerted`를 채울 수 없다 → 마이그레이션 = core 단독 소유). `docs/91` Q-67 |
