...
- **남은 것**: ① 봇 명령(/status·/queue·/mute)·**인라인 버튼 콜백**(Q-15 승격을 텔레그램에서) = 인바운드 핸들러(webhook/폴링) 미구현 — SEC-03 인바운드 화이트리스트도 그때(Q-61). ③ 현재가(네이버, Q-3). ~~④ 관리 알림~~ ✅ 해소(Q-56 — `AdminNotifier`·`PipelineHealthMonitor`, 연속 실패 push).
...
- **SEC-03 절반 해소(2026-07-21)**: **아웃바운드**(알림 발송)는 설정된 `TELEGRAM_CHAT_ID` 하나로만 나간다(`TelegramAlertSender`) — 다른 chat으로 새지 않는다. **인바운드**(봇 명령·인라인 버튼 콜백의 chat_id 화이트리스트·서명 검증)는 **여전히 미구현** — 인바운드 핸들러 자체가 없다(Q-20 ①). `TELEGRAM_ALLOWED_CHAT_IDS`는 그 핸들러가 생길 때 소비된다. 지금 인바운드 판정만 만드는 것은 여전히 투기다 — 버튼 기능과 함께 만든다.
