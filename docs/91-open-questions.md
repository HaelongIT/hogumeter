...
- **잠정값**: collector는 계속 `_derived`에 보존한다(생산자 쪽은 옳다). core는 태그를 무시한다. **고칠 수 없으니 세어서 노출한다** — `cycle` 이벤트에 `conditional=N`(조건부 딜 수, 0도 센다). 폴링을 켠 사람이 `docker logs`에서 오염률을 바로 본다.
...
- **재개 트리거**(무엇이 참이 되어야 하는가): ① `DealEvent`가 조건 태그를 지닐 수 있어야 한다 ② 딜 생성 시 원문의 태그가 그 필드로 흘러야 한다 ③ 기준가 표시·알림 본문이 그 태그를 **읽고 말해야** 한다. 셋 다 **core 기존 파일**(`DealEvent`·`DealEventEntity`·`IngestDealsUseCase`·`BenchmarkView`)이라 상대와 조율. **실 폴링을 켜기 전에 처리해야 한다** — 켜는 순간 표본이 조용히 오염된다(`pre-deploy §F`).
