...
- **잔여(여전히 열림)**: ① **UNCLASSIFIED 승격**은 사람이 variant를 골라야 해 아직 막는다(400 `REVIEW_PROMOTE_UNSUPPORTED`) — 후보 선택 입력 경로가 생기면 딜 생성으로 배선. ② **web 승격·기각 버튼**은 미착수(프론트 UI는 사용자 지휘 대기) — 지금 web `review/`는 "못 한다"고 말하는데 백엔드는 이제 된다. 버튼 추가 시 그 문구를 지운다. ③ resolve-then-recur: dedup_key unique-global이라 기각 후 같은 근거 재발생 시 기각 행 occurrences만 증가(재오픈 안 함) — 수용(보수적).
...
- **①②는 여전히 열림 + 재평가**: ① 세 컬럼 매핑은 **소비처가 없어 지금 하면 죽은 컬럼이 된다** — `exclude_keywords`는 Q-28(표본 적용, 제목 접근+GlobalSetting 필요), `demand_axis_filter`는 Q-66(수요축, web 등록 대기), `k_display`는 per-variant 기준가 K 오버라이드 기능이 있어야 산다. 매핑만 하면 "저장되는데 안 쓰이는" 손잡이라 원칙 위반. 각 소비 기능과 함께 매핑한다. ② `DEFAULT_PERIOD_MONTHS` 단일소유는 **어댑터가 그 값을 보고할 소비처가 있을 때** 한다(지금 GET은 미설정 시 숫자를 말하지 않기로 결정 — 과대약속 금지). 소비처 없이 상수만 옮기는 것은 투기다.
...
## [열림] Q-66. 수요축(demand axis) 기능이 통째로 죽어 있다 — 그런데 화면은 "분리" 손잡이를 그린다
...
- **🔴 실측(2026-07-11) — 죽은 것들의 사슬**:
  ...
  2. **`product_axis`는 쓰기만 하는 테이블**이다. `RegisterProductUseCase`가 저장하고, `ProductAxisRepository.findByProductId`는 **호출자가 0**이다. `GetProductsUseCase.ProductSummary`에 축이 없어 web으로 돌아오지도 않는다.
  3. **`DemandAxisMode.SPLIT`은 저장되지만 아무 동작도 바꾸지 않는다.** 기준가 표본 분리도, variant 분리도 없다. `demandAxisMode`를 읽어 분기하는 프로덕션 코드가 하나도 없다.
  4. **"SPLIT 필수"가 강제되지 않는다.** `RecordPurchaseCommand`·`Purchase`의 javadoc은 "demandAxisValue: SPLIT 필수·GROUPED 선택"이라 적어 두었지만, `RecordPurchaseUseCase`는 제품의 모드를 보지도 않는다. 산문은 GREEN일 수 없다.
...
- **잠정값(2026-07-11)**: 화면이 **사실을 말한다** — "분리(SPLIT)는 아직 표본을 나누지 않습니다 — 값은 저장되지만 동작은 묶음과 같습니다." + "가격에 영향 없는 축(색상 등)을 넣으면 표본이 쪼개집니다." 손잡이는 확정본에 있으므로 지우지 않는다(임의 변경 금지). 테스트가 두 문구를 요구한다(`RegistrationPage.test.tsx`). **구현되면 그 두 문단만 지운다(seam).**
- **재개 트리거**(무엇이 참이 되어야 하는가): ① 기준가 표본 조립이 `demandAxisMode`를 읽어 SPLIT일 때 수요축 값별로 분포를 나눈다 ② 등록이 축마다 유형(가격/수요)을 받고, variant는 **가격축만으로** 생성된다 ③ `demandAxisValue`가 SPLIT 제품에서 필수로 검증된다. ①은 `BenchmarkCalculator`(core 기존 파일), ②는 web + `RegisterProductUseCase`, ③은 `RecordPurchaseUseCase` — 전부 상대 소유이거나 조율 필요.
- **관련**: `check-table-wiring.sh`는 `product_axis`를 통과시킨다(엔티티가 있으므로 "이름이 나타난다"). 그 게이트의 명시된 한계다 — 읽기만/쓰기만 하는 테이블은 사람이 찾는다.

## [열림] Q-67. AL-03 후속 알림이 통째로 없다 — 로드맵은 "후속 자격 ✅ GREEN"이라 적었다
...
- **재개 트리거**(무엇이 참이 되어야 하는가): ① 딜의 상태·가격 변화가 **이벤트로 관찰**된다(`ReprocessDealPricesUseCase`·`ReprocessDealStatusUseCase`가 이미 변화를 감지하지만 알림을 태우지 않는다) ② 이미 알린 딜인지 알 수 있어야 한다(`alreadyAlerted` — 알림 이력 저장소가 없다. `deal_event`에 컬럼도 없다) ③ `AlertSender`가 후속 메시지를 낼 수 있어야 한다(어댑터는 Q-20 대기).
  - **②가 진짜 블로커다**: 알림 이력이 없으면 `shouldSendFollowUp`의 `alreadyAlerted`를 채울 수 없다. 스키마 추가(마이그레이션 = core 단독 소유)가 선행한다.
- **관련**: Q-20(봇 토큰) · Q-27 ③(품절 딜 오알림 — 그 반대 방향의 결함).
...
- **잠정값**: NULL 유지. 매칭은 이산 판정(CONFIRMED 등)으로 충분히 동작하며 confidence 없이도 파이프라인이 돈다. 컬럼은 스키마에만 있다.
...
## [열림] Q-72. M2 어댑터 미배선 — V3 스키마는 섰으나 생산자가 막혔다
...
- **잠정값(생산자별 막힘 사유)**:
  - `used_listing_observation` — 생산자=collector 번개 폴링. **검색어 소스(UsedSearch read 경로)와 실 네트워크가 정지조건**(Q-44 status 코드표·Q-4). collector엔 DB read 경로가 없다(현재 sink는 쓰기 전용).
  - `listing` — 생산자=목록 diff 유스케이스. `used_listing_observation` 소비가 선행이라 collector에 종속.
  - `listing_note`·`comparison_axis`·`listing_axis_value` — 생산자=메모/축 REST(USED-05). **막히지 않음** — UsedSearch 등록처럼 core REST로 배선·MockMvc 검증 가능. 다음 증분.
- **재개 트리거**(무엇이 참이 되어야 하는가): 메모/축 REST는 지금 착수 가능(다음 증분). observation·listing은 collector 실 폴링 승인 + UsedSearch read 어댑터(01-arch §열린 이음새 1)가 서야 배선된다. 각 테이블이 배선되면 allowlist에서 그 줄을 지운다(낡은 면제는 다음 결함을 숨긴다).
