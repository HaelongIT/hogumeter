...
## [해소 2026-07-27] Q-33. DIGEST 섹션 조립·저장 원자성·발송 — Q-81로 흡수, 6섹션 전부 완료
- **맥락(당시 스냅샷, 이제 낡음)**: 작성 당시엔 `DigestWindow`·`DigestRules`만 있고 섹션 조립·발송·스케줄은 전부 미착수였다. 이후 이 작업은 실제로 **Q-81** 항목 아래서 2026-07-24~27에 걸쳐 증분별로 진행·기록됐는데, 이 Q-33 항목은 그동안 갱신되지 않고 "[열림]"인 채 남아 있었다 — 같은 기능을 추적하는 두 항목이 갈라져 한쪽이 낡은 채로 board hygiene을 해친 사례.
...
- **✅ ① 해소(2026-07-24)**: `RecordDigestSentUseCase.recordSent(variantId, color, context, basisMode)`가 `digest_state`의 첫 production writer다. 색·문맥·basis 모드는 **스스로 계산하지 않고 파라미터로 받는다** — 그 도출은 섹션 조립(②)의 몫이라 아직 없다. `variant_id`가 수동 할당 `@Id`라 `save()`가 존재 여부와 무관하게 upsert(merge)한다 — 재기록 시 새 행이 안 생기는 것과, `ComputeDigestWindowUseCase`가 그 시각을 다음 창 시작으로 정확히 되읽는 왕복을 테스트가 증명한다.
...
- **✅ 섹션 ⑥ 조용한 주 해소(2026-07-25)**: `ComputeDigestQuietWeekUseCase.isQuietWeek(variantIds)` — `DigestRules.isQuietWeek`(마지막 호출자 0이던 순수 규칙)의 첫 소비자. variant별 창을 구해(창·기회수·전환 유스케이스 조합) 어느 variant에도 플로우·전환이 없으면 조용한 주. **anyGap은 상수 false 고정**(결정대로) — `ANY_GAP` 한 상수가 관측시계 연결 seam. 한계: 수집이 멈춘 주도 "조용한 주"로 보고될 수 있다. 이로써 `DigestRules` 두 메서드가 모두 소비된다(domain-consumers 게이트 초록).
- **✅ 조립 해소(2026-07-25, 부분)**: `AssembleVariantDigestUseCase.assemble(variantId)` → `VariantDigestRow`(창·① 최고 기회·③ 관찰 경과·② 전환). 여태 만든 섹션 유스케이스들이 **같은 창**으로 한 행에 묶이는 첫 소비자(각각은 테스트만 불렀다 — 소비자를 만들어 생산자를 검사).
- **✅ 다이제스트 전체 조립 해소(2026-07-25)**: `AssembleDigestUseCase.assemble()` — 등록된 모든 variant를 순회해 `VariantDigestRow` 리스트 + ⑤ `GetReviewQueueUseCase.pending()` + ⑥ `ComputeDigestQuietWeekUseCase.isQuietWeek()`를 `Digest` 하나로 묶는다. `AssembleVariantDigestUseCase`(variant 1건)의 배치판.
- **✅ 렌더링 1차 해소(2026-07-25)**: `DigestFormatter`(adapter/telegram, 순수) + `RenderDigestUseCase`(이름 배선, IO). 조용한 주는 한 줄로 축약(DIG-01 안전망 취지 — "발생"이 아니라 "가시화"). 아니면 이번 창에 기회·전환·관찰 중 하나라도 있는 variant만 개별 줄, 완전 무변동은 "나머지 N개 항목은 변동 없음"으로 합산(DIG-05 "정보 가치순 + 무변동 합산"의 첫 해석). ⑤는 건수 + 대상 미상(미귀속) 건수 병기. **이번 판의 명시적 한계**(다음 항목에 이어 해소):
...
- **✅ ④ 핀 결말 전이 + 부활 이벤트 해소(2026-07-27)** — WATCH(M6) 백엔드가 2026-07-25 안착해 재개 트리거가 충족됐다. `ComputeDigestPinEndingsUseCase.pinEvents(variantId, window)` — "핀 이력 딜"(WatchItem 존재, 상태 무관 — Q-83이 쓰는 것과 같은 정의) 범위로 부활을 좁힌다. 결말은 BOUGHT·MISSED만(기각 DROPPED는 원문이 제외), 판정 시각은 `WatchItem.resolvedAt`(결말)·`deal_alert.sent_at`(부활, DN-C1 REOPENED 후속). 둘 다 창(반개구간) 소속으로 이번 창 담김을 가른다. `AssembleVariantDigestUseCase.VariantDigestRow`에 `pinEvents` 필드 추가(같은 창으로 다른 세 섹션과 함께 조립), `DigestFormatter`가 "📌 샀어요/놓쳤어요/↩️ 다시 살아남"으로 렌더 — 이 섹션만 있어도(다른 신호 없이도) 무변동 합산 대상에서 빠진다.
- **🐛 부수 발견 — deal_alert.kind CHECK 제약이 REOPENED를 몰랐다(V20으로 수정)**: 이 증분의 통합 테스트가 `alerts.save(new DealAlertEntity(dealId, "REOPENED"))`에서 `DataIntegrityViolationException`을 냈다. 원인: `V4__alert_history.sql`의 `check (kind in ('FIRST','VERIFIED','PRICE_CHANGED','ENDED'))`가 DN-C1(2026-07-25, `FollowUpKind.REOPENED` 도입) 때 갱신되지 않았다 — **부활 후속 알림은 도입된 날부터 매번 이 제약에 걸려 실패하고 있었다**(`PipelineScheduler.runStep`이 삼켜 조용히 stepFailures만 늘었다, Q-56). `V20__deal_alert_reopened_kind.sql`이 제약을 갱신했고, `FollowUpAlertUseCaseTest#sendsReopenedFollowUp`(V20 없이 RED 확인)로 회귀를 잠갔다. **DIGEST④를 만들다가 우연히 찾은, DIGEST와 무관한 진짜 프로덕션 결함**이었다.
- **부수 배선**: `deal_alert.sent_at`을 읽기 전용으로 매핑(`insertable=false,updatable=false` — DB `default now()` 보존, INSERT 관여 안 함). `dead-columns-allowlist.txt`에서 이제 낡은 면제(`deal_alert.sent_at INTENTIONAL`)를 제거(게이트가 "낡은 면제" 능동 차단으로 잡음).
...
- **🔍 실측 추가(2026-08-05)**: `DigestWindow.occurredBeforeWindow`(docs/18 "발생은 N일 전 병기"의 재료로 지어진 순수 메서드)가 **호출자 0**이다 — `Matcher.confirm`·`OutlierDetector.isAbsurdVsCurrent`와 같은 방법(도메인 메서드별 호출수 grep)으로 찾았다. **그런데 이건 그 둘과 달리 지금 당장 배선해도 무의미하다**: `ComputeDigestBestOpportunityUseCase.bestOpportunity`가 후보를 `window.contains(d.firstSeen())`로 이미 걸러 놓아, 그 필터를 통과한 딜은 정의상 `occurredBeforeWindow(firstSeen)`가 **항상 false**다(같은 `firstSeen`을 "가시화 시각 근사"로도 "발생 시각"으로도 같이 쓰고 있어서 — 위 ③의 "가시화 시각 근사" 한계와 정확히 같은 원인). 지금 호출부만 이어 붙이면 절대 안 켜지는 죽은 분기를 만드는 "우연히 옳은 코드" 함정이라 하지 않았다.
- **재개 트리거**: 문서화된 한계들은 사람이 "이제 정확히 하자"고 판단할 때(예: 전역 quiet hours 설정 도입, 다중 구매 대표값 정책 확정) — 정지조건 아님. `occurredBeforeWindow`는 "가시화 시각 근사"(③) 자체가 실제 자격 획득 이력으로 교체될 때 — 그때만 `firstSeen`과 다른 진짜 가시화 시각을 넣어 뜻이 생긴다.
- **관련**: `docs/18`, `working-area/2nd-plan-intake.md`(원문 근거), `docs/17`(핀 이력 딜 정의), `V20__deal_alert_reopened_kind.sql`.
...
- **✅ 곁들여 해소(2026-08-05)**: API 필드 소비처 0 재감사에서 `UsedSearchView.exclude`·`.bonusGroups`가
  0이었다 — 등록 폼은 이 값을 만들어 보낼 줄 알았지만, "등록된 중고 검색" 목록은 `required`·`targetPrice`·
  `pollIntervalMin`만 되읽어 보여주고 제외 키워드·보너스 그룹은 되읽지 않았다(검색이 여럿이면 어느 검색에
  뭘 걸어 뒀는지 다시 볼 방법이 없었다). `UsedSearchPage.tsx`의 목록 항목에 "· 제외: …"·"· 보너스: …(모드)"
  줄 추가(둘 다 비어 있으면 그리지 않는다). 테스트 2건(있음/없음) + 뮤테이션(렌더링 제거 → 신규 테스트만
  RED 확인 후 복원).
