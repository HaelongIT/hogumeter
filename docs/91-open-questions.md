...
- **잔여 한계(열린 이유)**:
  ...
  4. **펨코 구조화 경로(`_hotdeal_price`)의 배송 조건 소실**: fixture엔 유료배송(금액) 사례가 없어 D2가 재현되지 않지만, `와우무배`(쿠팡 와우 회원)·`1만5천원무료`(조건부 무료배송) 같은 **조건**이 태그 없이 0으로 흡수된다. 와우가/일반가 손잡이(docs/90 §8)와 정합 필요.
...
- **진행(2026-07-09, 뽐뿌 실측)**: 뽐뿌 목록의 `.baseList-time`은 **당일 글 `21:10:11`(시:분:초) / 이전 글 `26/07/03`(YY/MM/DD)** 로 형식이 갈린다. 즉 목록만으로 postedAt을 얻으려면 **"오늘" 날짜를 주입**해야 하고(순수 파서 시그니처가 깨진다), 당일 글은 초 단위까지, 이전 글은 **날짜만**(→ 잠정값의 "날짜만이면 23:59 KST" 규칙이 발동). 현재 `parse_ppomppu`는 `posted_at=None`으로 두고 `capturedAt` 폴백에 맡긴다. 루리웹·펨코 목록의 시각 형식은 아직 미실측.
