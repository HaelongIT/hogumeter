...
- **①의 남은 둘 · ②는 여전히 열림**: ① `exclude_keywords`는 Q-28(표본 적용 — **제목 접근**과 GlobalSetting 읽기 경로가 필요), `demand_axis_filter`는 Q-66 ①(SPLIT 분포 분리)이 있어야 산다. 매핑만 하면 "저장되는데 안 쓰이는" 손잡이라 원칙 위반 — **각 소비 기능과 함께** 매핑한다(K가 그 선례다). ② `DEFAULT_PERIOD_MONTHS` 단일소유는 **어댑터가 그 값을 보고할 소비처가 있을 때** 한다(지금 GET은 미설정 시 기간을 숫자로 말하지 않기로 결정 — 과대약속 금지). 소비처 없이 상수만 옮기는 것은 투기다.
