...
- **남은 것**:
  ...
  - **번개 폴링 활성화** — `robots.txt`가 **한 번도 확인된 적이 없다**(2026-07-22 발견). 파서·fixture·적재 경로는 다 있는데 그것만 없다. 루리웹에서 정확히 그 가정 때문에 금지된 URL을 크롤링했으므로 확인 전엔 레지스트리에 넣지 않는다.
- **재개 트리거**(무엇이 참이 되어야 하는가): **사람이 `ALLOW_REAL_ROBOTS=1 bash scripts/check-robots.sh`를 돌려 번개가 ALLOW로 나오는 것**(대상에 추가됨). 그 뒤 마켓 폴링 루프 배선 + 실 폴링 승인. 각 테이블이 배선되면 allowlist에서 그 줄을 지운다(낡은 면제는 다음 결함을 숨긴다).
