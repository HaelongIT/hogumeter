...
- **남은 것(진짜 코드 밖 아님 — 다음 증분 대상)**:
  1. DIG-04 6개 섹션 조립(딜 요약·전환·관찰 경과·핀 결말〔WATCH 유보 종속〕·큐·조용한 주) — 각각 SIG/CAD/PUR/USED 데이터를 조합해야 한다. 조립 결과(색·문맥·basis 모드)를 ①(`RecordDigestSentUseCase`)에 넘긴다.
  2. 발송(텔레그램, quiet hours 존중, 분할 발송 시 절단 금지·연번). 발송 성공 후에만 ①을 부른다(원자성).
  3. 스케줄(기본 일요일 20시 KST, 사용자 설정 가능).
- **재개 트리거**: 없음 — 순수하게 구현 분량 문제라 다음 무중단 증분에서 이어간다(사람 결정·외부 키 대기 아님). 순서 권장: 섹션 조립(가장 간단한 섹션부터: ③ 관찰 경과, ⑥ 조용한 주) → 발송 → 스케줄.
