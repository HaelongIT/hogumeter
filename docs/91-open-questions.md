...
## [해소(코드) 2026-07-22] Q-73. 텔레그램 인라인 버튼 피드백이 약하다 — 토스트는 있으나 놓치기 쉽고, 메시지·버튼은 안 변한다
- **✅ 코드 해소(2026-07-22, ①②③ 셋 다)**: 아래 세 지점을 전부 고쳤다. 페이크로 검증 완료 · core 전체 GREEN. **실 응답(모달 표시·메시지 편집)은 재배포 후 버튼 1회로 수동 스파이크**(실 발송이라).
...
- **남은 것**: 실 스파이크(재배포 후 버튼 1회로 모달·편집이 실제로 보이는지). 코드는 완결.
