...
- **왜 지금 안 하나**: 1인용·저빈도(게시판당 1req/min)라 실피해가 낮고, 상한을 잘못 잡으면 **정상 제목이 잘려 매칭이 깨진다**(BM-03). 자르는 것과 거절하는 것 중 무엇이 옳은지도 실 데이터 없이는 정하기 어렵다.
- **잠정값**: 상한 없음. 파서가 낸 값을 그대로 적재한다.
- **재개 트리거**: 실 수집에서 비정상 크기(예: 제목 1KB 초과, `raw` 1MB 초과)가 관측되면 — 또는 서비스화 검토 시(불특정 다수의 페이지를 받게 된다). 그때 **자르지 말고 이벤트로 남기고 건너뛰는 쪽**을 먼저 검토한다(정직성: 잘린 제목은 정상 제목의 얼굴을 한 거짓말이다). seam = `pipeline/ingest.py`의 `to_raw_records` 1곳.
