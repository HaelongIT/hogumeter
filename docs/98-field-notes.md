...
| **`SIG-02`** | ✅ **해소 2026-07-23** — `site_poll_state`(V12) + `ObservationClock`으로 배선. ⚠️ 이 칸에 있던 옛 진단("확신 한 칸만 도달 가능 / 수집이 멈춰도 강등 안 됨")은 **방향이 반대였다**: `lastPoll = now`는 벽시계라 staleness가 자랐고, 수집이 멈춘 동안 **딜이 늙은 것처럼 보여 거짓 강등**됐다. 진단을 코드로 재현하지 않고 적으면 이렇게 굳는다 — Q-34 참조 |
| `SIG-03` | 목록 표면·📌 = M4 |
...
## 2026-07-22 robots.txt 실 대조 (SEC-08, pre-deploy §F) — 실 폴링 직전
...
- **도구가 스스로 강제한다(코드 변경 불필요)**: `HttpFetcher.fetch`가 `robots.allows(url)`를 먼저 보고 거짓이면 **요청을 보내지 않고** `_ROBOTS_DISALLOWED=403`을 돌린다 → `classify_status`가 `BLOCKED` → `loop`가 그 사이트를 "차단 신호 감지: 자동 중지, 재시도하지 않음"으로 끈다. 즉 실 폴링을 켜도 펨코로는 **단 한 번도 나가지 않는다**(robots.txt 조회 제외).
- **그래서 M1 실 수집 소스는 2사(뽐뿌·루리웹)다.** 표본이 3사 가정보다 적으므로 기준가 n·교차검증 m이 그만큼 느리게 쌓인다 — "2개 사이트 검증"이 사실상 최대 교차검증이다.
- **펨코 재개 조건**: robots가 바뀌거나 공식 API·제휴 경로가 생길 때만. `decisions-needed` D-3(차단 사이트 재개 경로)과 별개다 — D-3은 *차단당한* 경우이고 이건 *처음부터 금지*된 경우다.
- 출력 산문이 콘솔에서 깨졌다(cp949) — 판정·상태 같은 **ASCII 마커는 멀쩡했다.** "출력 라벨은 ASCII로"(docs/99)가 예측한 그대로다.

## 2026-07-22 실 폴링 첫 사이클 실측 (뽐뿌·루리웹 2사) — 루리웹 제목 잘림이 가격을 먹는다

첫 실 수집 52건(ppomppu 24 · ruliweb 29 기준 시점 표본)에서 **처음으로 측정**된 값. 그동안 "실 표본이 없어 못 정한다"던 것들의 근거다.

| site | 전체 | 잘린 제목(`…`로 끝남) | 가격 미검출 | **잘려서 가격 유실** | 잘림률 |
|---|---|---|---|---|---|
| ruliweb | 29 | 9 | 10 | **5** | **31.0%** |
| ppomppu | 24 | 0 | 0 | 0 | 0.0% |

- **루리웹은 목록 뷰에서 제목을 자른다**(`...`). 그 바람에 가격이 **숫자 중간에서** 끊긴다: `/ 115...`, `90% 할인 3,14...`, `마우스 / 26...`, `게이밍 마우스 19,00...`, `30% 할인 (15,4...`.
- **결과: 루리웹 딜의 ~17%(5/29)는 가격을 못 읽어 `skippedNoPrice`로 빠진다** → `deal_event`가 안 생기고 → **알림이 영원히 안 간다.** 등록 제품의 특가가 하필 긴 제목이면 통째로 놓친다(놓침).
- 잘린 9건 중 4건은 가격이 잘림 지점 **앞**에 있어 정상 파싱됐다 — 잘림이 곧 유실은 아니다.
- **뽐뿌는 잘림 0%** — 목록에 전체 제목이 나온다. 사이트별로 성질이 완전히 다르므로 **합산 통계로 보면 안 된다**(합산 19%는 뽐뿌를 억울하게 만들고 루리웹 문제를 희석한다).
- **이것이 `raw_deal_post.body_text`(Q-18, 죽은 컬럼 게이트가 물고 있던 그 컬럼)의 존재 이유다.** 잘린 제목의 가격은 **글 본문/상세 페이지**에서 복구해야 한다 — 지금 collector는 목록만 폴링해 그 컬럼이 항상 null이다.
- **미결(결정 필요)**: 상세 페이지 fetch는 요청 수를 크게 늘린다(루리웹 사이클당 1 → 1+9). 저빈도·개인용 원칙(절대 원칙 5)과 맞바꿈이라 **사람이 정할 사안**이다. 후보: ① 잘린 제목(`…`로 끝남)만 상세 fetch(사이클당 ~9, 여전히 10배) ② 등록 제품 별칭이 제목에 걸리는 것만 상세 fetch(대개 0~1건, **가장 싸고 목적에 정확히 부합**) ③ 안 함(루리웹 긴 제목 딜은 포기).

### 2026-07-22 robots 재대조 — **고친 매처로 확정**(위 표를 대체한다)

와일드카드 지원을 넣은 뒤 같은 명령(`ALLOW_REAL_ROBOTS=1 bash scripts/check-robots.sh`)을 다시 돌린 결과. **이것이 정본이다.**

| 사이트 | 판정 | 근거 |
|---|---|---|
| **ppomppu** | **✅ ALLOW** | 고친 매처 기준. 우리 URL(`/zboard/zboard.php?id=ppomppu`)에 걸리는 규칙 없음 |
| **ruliweb** | **🔴 DISALLOW** | `Disallow: /*view=` — 우리 목록 URL의 `?view=thumbnail`이 걸린다 |
| **fmkorea** | **🔴 DISALLOW** | `/hotdeal` |

- **게이트가 스스로를 증명했다**: 수정 전 같은 명령이 루리웹을 ALLOW로 냈고, 수정 후 DISALLOW로 낸다. 실 대상의 실 정책 파일이 회귀 테스트 역할을 했다.
- **M1 실 수집 소스는 뽐뿌 1사다.** 교차검증(m)이 **불가능**해진다 — `cross_verified`는 2개 이상 사이트에서 같은 딜을 봐야 서므로, 1사 체제에서는 모든 딜이 "미검증(단독)"이다. 기준가 tier·알림 강도 라벨이 그 사실을 이미 말한다(지어내지 않는다).
- **루리웹 복구 경로**: `view=` 없는 URL(`/market/board/1020`)은 허용된다. 그 뷰의 HTML 구조가 다를 수 있어 golden fixture를 새로 받아야 한다(파서는 `view=thumbnail` 기준으로 짜였다).
- **레지스트리에서 뺀다**: 금지된 사이트를 스케줄에 두면 켤 때마다 게이트가 막고 SEC-08이 "차단 감지 → 자동 중지 + 관리 알림"을 낸다 — 이미 아는 사실로 알림을 울릴 이유가 없다. 게이트는 그대로 두 번째 방어선으로 남는다(누가 다시 추가해도 요청은 안 나간다).

### 2026-07-22 뽐뿌 실 표본 111건 파서 감사 — **가격 미검출 0%**, Q-64 위험은 사실상 없음

실 폴링 첫 시간의 뽐뿌 크롤링분만(테스트 삽입분 제외: `url like '%ppomppu.co.kr%'`) 전수 측정. golden(21건)이 아닌 **실 표본으로는 처음**이다.

| 지표 | 값 | 판정 |
|---|---|---|
| 실 크롤링 | 111건 | — |
| **가격 미검출** | **0건 (0%)** | 루리웹 17% 유실과 극명한 대조 — 뽐뿌는 제목을 안 자른다 |
| 1천원 미만 | 1건 | `[ios]스펀지밥 외 아이폰게임(400원,무료)` — **진짜 싼 딜**, 오검출 아님 |
| 5백만원 초과 | 0건 | 모델번호·용량을 가격으로 읽는 오검출 없음 |
| `배송비미상` 태그 | 4건 (3.6%) | 전부 옳음 — `유클무료`·`네멤무료`·`네맴무배`(오타 변형 포함) = **멤버십 조건부** |
| 배송 표기 아예 없음 | **2건 (1.8%)** | `캠프통 무제한이용권`(디지털·서비스 → 0이 진실) · `프로스펙스 …가격 다양`(범위 표기) |

- **숫자 배송비 합산이 실 데이터로 검증됐다**: `(19,750원~/3,000원)` → **22,750**, `(5,460원/3,500원)` → **8,960**. 2026-07-10에 고친 "숫자 배송비가 조용히 버려지던" 결함이 실전에서 옳게 동작한다(BM-02 실결제가+배송비).
