...
echo "── 차단되어야 함 (exit 2) — 파괴적 git push ──"
# 리터럴로 쓰면 가드가 이 테스트 자신을 막는다. 조립해 쓴다(아래 GP도 같은 이유).
P="$(printf 'push')"
check 2 "{\"tool_input\":{\"command\":\"git $P --force origin main\"}}"        "force-push"
check 2 "{\"tool_input\":{\"command\":\"git $P --force-with-lease origin main\"}}" "force-with-lease"
check 2 "{\"tool_input\":{\"command\":\"git $P -f\"}}"                          "단축 -f"
check 2 "{\"tool_input\":{\"command\":\"git $P -uf origin main\"}}"             "묶음 단축 -uf"
check 2 "{\"tool_input\":{\"command\":\"git $P --delete origin feature\"}}"     "원격 브랜치 삭제"
check 2 "{\"tool_input\":{\"command\":\"git $P -d origin feature\"}}"           "단축 -d 삭제"
check 2 "{\"tool_input\":{\"command\":\"git $P --mirror\"}}"                    "mirror(원격 전체 덮어쓰기)"
check 2 "{\"tool_input\":{\"command\":\"git $P --prune origin\"}}"              "prune(원격 참조 제거)"
check 2 "{\"tool_input\":{\"command\":\"git $P origin +main\"}}"                "force refspec (+main)"
check 2 "{\"tool_input\":{\"command\":\"git $P origin :main\"}}"                "delete refspec (:main)"
check 2 "{\"tool_input\":{\"command\":\"cd /tmp && git $P --force\"}}"          "체이닝 뒤 force-push"
check 2 "{\"tool_input\":{\"command\":\"git $P origin main; git $P --force\"}}" "두 번째 push가 force"
check 2 "{\"tool_input\":{\"command\":\"git -c core.hooksPath=/dev/null $P -f\"}}" "git -c 옵션 뒤 force"

echo "── 통과해야 함 (exit 0) — 일반 git push ──"
# 사용자 결정(2026-07-10): 일반 푸시는 허용, 파괴적 형태만 금지. 여기가 더 중요하다 —
# 오차단은 "푸시를 못 한다"로 조용히 되돌아간다.
check 0 "{\"tool_input\":{\"command\":\"git $P origin main\"}}"                 "일반 푸시"
check 0 "{\"tool_input\":{\"command\":\"git $P\"}}"                             "인자 없는 푸시"
check 0 "{\"tool_input\":{\"command\":\"git $P -u origin main\"}}"              "-u (f/d 없음)"
check 0 "{\"tool_input\":{\"command\":\"git $P --set-upstream origin main\"}}"  "--set-upstream"
check 0 "{\"tool_input\":{\"command\":\"git $P --dry-run origin main\"}}"       "--dry-run"
check 0 "{\"tool_input\":{\"command\":\"git $P --follow-tags\"}}"               "--follow-tags (f 포함하나 -- 옵션)"
check 0 "{\"tool_input\":{\"command\":\"git $P origin main:main\"}}"            "동명 refspec (콜론 중간)"
check 0 "{\"tool_input\":{\"command\":\"git commit -m \\\"--force 는 금지다\\\"\"}}" "커밋 메시지 안의 --force"
check 0 "{\"tool_input\":{\"command\":\"git log --oneline origin/main..main\"}}" "미푸시 목록 확인"

echo "── 오차단 회귀 (exit 0) — 초판이 전부 막았던 것들 ──"
GP="git$(printf ' ')push"
check 0 "$(printf '{"tool_input":{"command":"grep -rn \\"%s --force\\" docs/"}}' "$GP")"           "grep으로 force-push 문구 검색"
