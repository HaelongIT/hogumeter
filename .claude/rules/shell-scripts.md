...
- **"설정이 옳다"와 "설정이 옳게 유지된다"는 다른 계약이다.** `.gitignore`·compose·nginx.conf가 지금 옳다는 사실은 내일도 옳다는 뜻이 아니다 — 옳음을 실행 가능한 계약으로 잠근다(gitleaks는 gzip 안을 못 본다: `backups/` 한 줄이 지워지면 DB 덤프가 커밋되고 CI는 통과한다).
