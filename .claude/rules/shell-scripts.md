...
## 계약 테스트의 함정 (게이트를 시험하는 코드도 코드다)

- **`$(fn ...)` 안의 대입은 부모로 돌아오지 않는다.** 명령 치환은 **서브셸**이다 — 케이스마다 `case_no=$((case_no+1))`로 디렉토리를 나누려 했더니 모든 케이스가 `r1`을 재사용했고, **앞 케이스가 남긴 allowlist가 뒤 케이스를 조용히 면제해** 차단 케이스 넷이 전부 통과했다. 일회용 디렉토리는 `mktemp -d "$work/rXXXXXX"`로 만든다. (99: 2026-07-10)
- **게이트가 자기 계약 테스트를 우연히 면제하지 않는지 본다.** 테스트 데이터에 opt-in 문자열·스크립트 이름이 들어 있으면 게이트가 그걸 실행으로 읽는다 — **명시적으로 allowlist에 선언**한다. 우연히 옳은 코드는 다음 커밋에 틀려진다.
