...
- 확정본상 조회·비교 화면은 M4다. **읽기 전용 판단 화면(`decision/`)만 앞당겼다** — M1 잔여 블로커가 전부 외부(토큰·키·승인)이고 core 변경이 0이라 되돌리기 쉽다(`decision-log` 2026-07-09). 승격 큐·비교는 여전히 후순위.
...
- 라우터를 들이지 않는다. 화면이 둘이라 `App.tsx`의 탭으로 족하다 — URL이 필요해지면 그때 넣는다.
