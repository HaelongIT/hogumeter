...
- **한 엔티티 안에서 같은 제네릭 소거 타입(`List<String>` 등)에 서로 다른 `@JdbcTypeCode`(ARRAY vs JSON)를 섞지 않는다** — Hibernate가 타입 기술자를 공유해 손대지 않은 다른 필드의 스키마 검증이 깨진다(`String[]`도 답이 아니다). jsonb 문자열 목록은 `Map<String,Object>`(단일 키로 감싸기)처럼 이미 검증된 다른 소거 타입을 쓴다. (99: 2026-07-30)
...
- **엔티티가 매핑하지 않는 컬럼은 네이티브 SQL로 다룬다** — 엔티티 매핑을 건드리지 않고 진실에 닿는 길이다(매핑을 붙이는 순간 위 delete+insert 함정에 노출된다). `GetReviewQueueUseCase`(읽기: `status`·`created_at`)와 `PreserveAppliedConditionsUseCase`(쓰기: `applied_conditions`)가 같은 수법이다. **"막혔다"고 적기 전에 이 길을 먼저 시도한다** — Q-46·Q-48·Q-50이 전부 그렇게 잘못 봉인돼 있었다. (99: 2026-07-10)
...
> TDD·순수 도메인·파라미터 주입·Flyway 단일 소유(`core/`에서만 마이그레이션)는 CLAUDE.md에 있다. 여기 옮겨 적지 않는다 — 중복 지침은 준수율을 떨어뜨린다.
