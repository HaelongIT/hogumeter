...
#
# ⚠️ 카운터를 **하나씩** 본다. 예전엔 `matched[...]` 괄호 전체를 한 문자열로 grep했는데, 카운터를
# 하나 더하자(`skippedForeignSource`) 값이 다 옳은데도 깨졌다 — 문구를 grep하는 검사는 문구를 굳히고,
# 굳은 문구는 "카운터를 늘리지 마라"는 뜻이 된다. 존재해야 할 값만 각각 단언한다.
for counter in 'matched\[' 'confirmed=1' 'candidate=0' 'unknown=0' 'rejected=0' 'skippedNoPrice=0'; do
...
# PurchaseObservation 6필드(reportCard 포함, CLOSED만 채움·그 외 null) + 중첩 ObservationContext 6필드 —
# 이름 하나만 바뀌어도 화면이 undefined를 그린다. null도 직렬화되므로 OBSERVING 응답에도 "reportCard":가 있다.
for field in purchaseId state paidPrice purchasedAt context reportCard; do
...
echo "--- 5-4) 중고 목록 스냅샷 → listing 생애주기 (USED-02, 접기) ---"
# `ListingDiff`는 순수 도메인에 있었지만 **호출자가 0**이었고 `listing`·`used_listing_observation`은
# 아무도 읽지 않는 테이블이었다 — 전부 GREEN인 채로. 이제 틱이 접는다. 그 배선이 스택에서 실제로
# 도는지는 모듈 테스트가 못 본다(컨테이너·스케줄러·트랜잭션 경계 밖).
#
# 두 배치를 심는다: T1(매물 2건) → T2(하나는 가격 인하, 하나는 소실). 접기가 옳게 돌면
# a1=820,000 ACTIVE / a2=SOLD 이고 배치 2개를 접었다고 틱이 말한다.
compose exec -T postgres psql -q -U "${DB_USER:-hogumeter}" -d "${DB_NAME:-hogumeter}" \
...
used_state() {
	compose exec -T postgres psql -qtA -U "${DB_USER:-hogumeter}" -d "${DB_NAME:-hogumeter}" \
		-c "select string_agg(listing_id || '=' || status || '/' || price, ',' order by listing_id) from listing" |
		tr -d '\r'
}
for _ in $(seq 20); do
