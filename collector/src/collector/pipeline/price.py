...
# BE-05(코드리뷰 20260806): `\d{4,}`는 탐욕적이라 뒤쪽 부정형 전방탐색(`_UNIT`)이 5자리+ 숫자에서
# 실패하면 엔진이 자리수를 하나씩 줄여 **백트래킹**한다 — 정확히 4자리면 가드가 통하지만 5자리
# 이상이면 4자리로 줄인 부분 문자열 뒤가 우연히 다음 숫자/문자라서 가드를 통과해 앞 4자리만 거짓
# 가격이 됐다(`i5-14600K`→1460원, `20000mAh`→2000원). 원자 그룹 `(?>...)`으로 백트래킹을 막는다 —
# Python 3.11+ 문법(pyproject.toml이 3.12를 요구한다).
