...
{"ts":"...","event":"cycle","sites_polled":3,"deals":69,"by_site":{"ppomppu":21,...},
 "failures":0,"blocked":0,"alerts":0,"conditional":7,"stopped_sites":[],"written":69}
...
**`conditional`을 보라.** 조건부 가격(`카할`=카드할인 · `유료배송(금액미상)` · 펨코 `조건부무료배송:…`) 딜 수다. 그 태그는 `raw._derived`까지만 가고 **`deal_event`에 도달하지 않아**(`docs/91` Q-46) 그만큼의 딜이 **무조건 가격인 척** 기준가 표본에 들어간다. 고칠 수 없는 결함이라 **세어서 노출한다** — 골든 실측 기준 뽐뿌 9.5% · 펨코 15%다. 이 값이 `deals`의 1할을 넘으면 기준가를 그대로 믿지 말 것.
