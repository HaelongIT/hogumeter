 ...
 * 미상 항목 승격은 {@code variantId} 쿼리 파라미터로 대상을 지정해야 한다(Q-15 ①, 2026-07-31) — 없으면
 * 400. 이상치는 {@code variantId}를 무시한다(이미 딜이 있다).
 */
@RestController
public class ReviewQueueController {
	...
	public void promote(@PathVariable long id, @RequestParam(required = false) Long variantId) {
		resolve.promote(id, variantId);
	}
